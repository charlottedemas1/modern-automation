...
2. Select the existing `vault-lab-iis` project

### Step 3: Deploy Vault Cluster

1. From the project overview page, click **Vault Dedicated - View Vault Dedicated**
2. On the Vault overview page, click **+ Create cluster**
3. Select Amazone Web Services
4. Configure the cluster settings:
   - **Vault tier**: Development
   - **Cluster size**: Extra Small
   - **Network**: Select the existing `hvn-iis` network
...
2. In the **Quick actions** pane, click **copy** under **New Admin Token**
...
> **Important**: Make sure you are in your HCP Vault configuration repository, NOT in example repositories like `learn-terraform-variables` or similar training repositories.

1. Add the following resource to your `main.tf`:
   ```hcl
   resource "aws_s3_bucket" "test_bucket" {
     bucket = "hcp-vault-test-{your_initials}"
     force_destroy = true
   }
   ```

### Step 7: Deploy with HCP Terraform

1. Commit and push your changes to trigger a new run via VCS integration
2. Monitor the plan output to confirm AWS authentication is working
3. Apply the changes.
